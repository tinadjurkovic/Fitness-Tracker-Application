...
import { StrengthenMusclesMealPlan, StrengthenMusclesMealPlanDay } from './strengthen-muscles.model';

@Component({
  selector: 'app-strengthen-muscles',
  templateUrl: './strengthen-muscles.component.html',
  styleUrls: ['./strengthen-muscles.component.css'],
})
export class StrengthenMusclesComponent implements OnInit {
  exercisePlan: any = {};
  showExercisePlan: boolean = false;
  showMealPlan: boolean = false;
  mealPlan: { [key: string]: StrengthenMusclesMealPlan } = {};
  ...
  ngOnInit(): void {
    fetch('assets/exercise-plan.json')
      ...
      fetch('assets/strenghten-muscles-meal-plan.json')
      .then((response) => response.json())
      .then((data) => {
        this.mealPlan = data;
