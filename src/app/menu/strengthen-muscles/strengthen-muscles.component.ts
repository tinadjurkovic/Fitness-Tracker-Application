...
import { StrengthenMusclesMealPlanDay } from './strengthen-muscles.model';
...
export class StrengthenMusclesComponent implements OnInit {
  ...
  mealPlan: { [key: string]: StrengthenMusclesMealPlanDay } = {};
  ...
  ngOnInit(): void {
    ...
    fetch('assets/strengthen-muscles-meal-plan.json')
      .then((response) => response.json())
      .then((data) => {
        this.mealPlan = data;
