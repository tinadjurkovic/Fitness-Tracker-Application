...
export class LoseWeightComponent implements OnInit, OnDestroy {
  user: any;
  ...
  ngOnInit(): void {
    ...
    this.mealPlanSubscription = this.mealPlanService.getMealPlan('lose-weight').subscribe(
      (data) => {
        this.mealPlan = data;
      },
      (error) => {
  ...
  goToTracker(): void {
    const selectedMenuOption = 'Lose Weight'; 
